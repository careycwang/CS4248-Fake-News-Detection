...
class Classify(torch.nn.Module):
    def __init__(self, params, vocab_size, ntags, pte=None):
        ...
        if params.encoder == 2:
            ...
            attention_dim = 2 * params.hidden_dim if params.bidirectional and params.attention else params.hidden_dim
            self.linear_transform = nn.Linear(in_features=attention_dim,
                                              out_features=ntags)
...
class LstmEncoder(torch.nn.Module):
    ...
    def forward(self, embeds, seq_lens):
        ...
        # hidden = torch.cat([cn[-1], cn[-2]], dim=1) if self.bidirectional else cn[-1]
        # energy, linear_combination = self.attention(cn[-1], outputs, outputs)
